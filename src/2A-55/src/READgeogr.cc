...
int Volume::READgeogrNew(void)
{
  ...
  int j = 0;
  FILE *fp;
  fp = fopen(params.inputGeoMapFn, "r");
  ...
  if ((xdim != params.xydim) || (ydim != params.xydim))
    {
	  ...
	  TKreportWarning(W_2A55_GEOG_DIM_INCORRECT);
      exit(FAILED_CODE1);
    }    
  ...
  if ((fpin3 = fopen(params.inputGeoMapFn, "r")) == NULL)
    {
      fprintf(stderr, "Sorry, can't open input file %s\n",
	      params.inputGeoMapFn);
	  TKreportWarning(W_2A55_CANT_OPEN_GEOMAP_FILE);
      exit(FAILED_CODE2);
    }
	  ...
	  if (test != 3)
	    {
		  ...
		  TKreportWarning(W_2A55_GEO_MAP_FORMAT_OBSOLETE);
            }
	  map_1km[jj][ii] = index;
        }
    }
	  ...
	  if ((ind_land == 0) && (ind_sea == 0))
	    {
		  ...
		  TKreportWarning(W_2A55_GEO_MAP_ERROR);
	      exit(FAILED_CODE3);
            }
	  else
	    {
