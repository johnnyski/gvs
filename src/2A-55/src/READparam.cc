 ...
 *  -------------------------------------------------------               */
	...
	TKreportWarning(W_2A55_OPEN_VALIDRANGE_IFILE_FAILED);
	exit(FAILED_CODE2);
      }
	...
	fprintf(stderr, "Problem with valid data range file %s!\n",
		params.inputValidRangeFn);
	TKreportWarning(W_2A55_INVALID_FORMAT_VALIDRANGE_IFILE);
	exit(FAILED_CODE3);
      }
    }
 ...
 *  -----------------------------------------------------                    */
	...
	TKreportWarning(W_2A55_OPEN_ZRTRAN_IFILE_FAILED);
        exit(FAILED_CODE2);
      }
 ...
 *  -----------------------------------------------------                    */
    ...
    if ((params.xydim > MAX_X_DIM) || (params.xydim > MAX_Y_DIM) || (params.zdim > MAX_Z_DIM)) {
		...
		TKreportWarning(W_2A55_GRID_DIM_TOO_BIG);
        exit(FAILED_CODE3);
    }
 ...
 *  --------------------------------------------------------------           */
   scanf("%d", &params.level);
	...
	fprintf(stderr, "Invalid analysis level: must be 0 <= level < %d!\n",
		params.zdim);
	TKreportWarning(W_2A55_INVALID_ANAL_LEVEL);
	exit(FAILED_CODE3);
      }
 ...
 *  --------------------------------------------------                       */
	...
	TKreportWarning(W_2A55_INVALID_MAXTYPE_LEVEL);
	exit(FAILED_CODE3);
    }
    if (params.maxtype == 1) {
 ...
 *  ----------------------------------------------------------------------   */
	...
	if (test != 2) {
		...
		TKreportWarning(W_2A55_INVALID_FORMAT_ZRTRAN_IFILE);
	    exit(FAILED_CODE3);
	}
	...
	TKreportWarning(W_2A55_INVALID_ZRTRAN_TYPE);
	exit(FAILED_CODE3);
    }
 ...
 *  ---------------------------------------------------------------------    */
	...
	TKreportWarning(W_2A55_INVALID_RADAR_EST_GS);
	exit(FAILED_CODE3);
    }
