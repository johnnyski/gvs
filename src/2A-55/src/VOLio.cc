...
/* TN 041296 -- Modified so vertical profile and cfad filenames are 
 * given by user..
 */
...
int Volume::readRadarVolume(const int nfield)
{
  ...
  params.second = mudfile.header.VolStart.Second;
  params.time   = params.hour * 10000 + params.minute*100 + params.second;
  ...
  params.lat    = mudfile.header.Lat.Deg * 10000 +
	mudfile.header.Lat.Min * 100 +
	mudfile.header.Lat.Sec;
  params.lon    = mudfile.header.Lon.Deg * 10000 +
	mudfile.header.Lon.Min * 100 +
	mudfile.header.Lon.Sec;

  fclose(fp);

  // Now scan for the threshold filename.

//  Replaced 960125 by SRB for 2A55 code:
//  sprintf(thres_fn, "/home/disk/hail/joe/radar/old/thresholds");
//  scanf("%s", thres_fn);
//  fprintf(stderr, "Threshold filename = %s\n", thres_fn);

    strcpy(thres_fn,params.inputThresholdFn);  // added 960125 SRB
	...
	if (!fp)
	  {
		...
		TKreportWarning(W_2A55_OPEN_THRESHOLD_FILE_FAILED);
	    exit(FAILED_CODE1);
	  }
	else
	  {
...
int Volume::writeASCII2a53(void)
{  
  ...
  // (Z) to rain (R) transformation. 
    ...
    if (params.ASCIIformat == 1)
      {
	for (int j = 0; j < params.xydim; j++)
	  {
	    for (int i = 0; i < params.xydim; i++)
	      {
...
int Volume::writeASCII2a54(void)
{  
...
int Volume::writeASCII2a55(void)
{  
...
*/
	sprintf(fn, params.outputMeanVertProfFn); //TN 041296
    FILE *fp = fopen(fn, "w");
    if (!fp)
      {
	fprintf(stderr, "Error opening ASCII output file.\n");
...
/* TN 041296
	sprintf(fn, "%s/%s.%s.%s.cfad%02d",
		...
		1);
*/
	sprintf(fn, "%s%02d", params.outputCFADFn, 1); // TN 041296
	FILE *fp = fopen(fn, "w");
	if (!fp)
	  {
	    fprintf(stderr, "Error opening CFAD ASCII output file.\n");
		TKreportWarning(W_2A55_OPEN_CFAD_ASCII_OFILE_FAILED);
	    exit(FAILED_CODE1);
	  }
...
/* TN 041296
	sprintf(fn, "%s/%s.%s.%s.cfad%02d",
		...
		2);
*/
	sprintf(fn, "%s%02d", params.outputCFADFn, 2); // TN 041296
	fp = fopen(fn, "w");
	if (!fp)
	  {
	    fprintf(stderr, "Error opening CFAD ASCII output file.\n");
...
/* TN 041296
	sprintf(fn, "%s/%s.%s.%s.cfad%02d",
		...
		3);
*/
	sprintf(fn, "%s%02d", params.outputCFADFn, 3); // TN 041296
	fp = fopen(fn, "w");
	if (!fp)
	  {
	    fprintf(stderr, "Error opening CFAD ASCII output file.\n");
...
/*  TN 041296
	sprintf(fn, "%s/%s.%s.%s.cfad%02d",
		...
		4);
*/
	sprintf(fn, "%s%02d", params.outputCFADFn, 4); // TN 041296
	fp = fopen(fn, "w");
	if (!fp)
	  {
	    fprintf(stderr, "Error opening CFAD ASCII output file.\n");
...
/*  TN 041296
	sprintf(fn, "%s/%s.%s.%s.cfad%02d",
		...
		5);
*/
	sprintf(fn, "%s%02d", params.outputCFADFn, 5); // TN 041296
	fp = fopen(fn, "w");
	if (!fp)
	  {
	    fprintf(stderr, "Error opening CFAD ASCII output file.\n");
...
/* TN 041296
	sprintf(fn, "%s/%s.%s.%s.cfad%02d",
		...
		6);
*/
	sprintf(fn, "%s%02d", params.outputCFADFn, 6); // TN 041296	
	fp = fopen(fn, "w");
	if (!fp)
	  {
	    fprintf(stderr, "Error opening CFAD ASCII output file.\n");
...
/* TN 041296
	sprintf(fn, "%s/%s.%s.%s.cfad%02d",
		...
		7);
*/
	sprintf(fn, "%s%02d", params.outputCFADFn, 7); // TN 041296
	fp = fopen(fn, "w");
	if (!fp)
	  {
	    fprintf(stderr, "Error opening CFAD ASCII output file.\n");
...
/* TN 041296
	sprintf(fn, "%s/%s.%s.%s.cfad%02d",
		...
		8);
*/
	sprintf(fn, "%s%02d", params.outputCFADFn, 8); // TN 041296
	fp = fopen(fn, "w");
	if (!fp)
	  {
	    fprintf(stderr, "Error opening CFAD ASCII output file.\n");
...
/* TN 041296
	sprintf(fn, "%s/%s.%s.%s.cfad%02d",
		...
		9);
*/
	sprintf(fn, "%s%02d", params.outputCFADFn, 9); // TN 041296
	fp = fopen(fn, "w");
	if (!fp)
	  {
	    fprintf(stderr, "Error opening CFAD ASCII output file.\n");
...
/* TN 041296
	sprintf(fn, "%s/%s.%s.%s.cfad%02d",
		...
		10);
*/
	sprintf(fn, "%s%02d", params.outputCFADFn, 10); // TN 041296
	fp = fopen(fn, "w");
	if (!fp)
	  {
	    fprintf(stderr, "Error opening CFAD ASCII output file.\n");
...
/* TN 041296
	sprintf(fn, "%s/%s.%s.%s.cfad%02d",
		...
		11);
*/
	sprintf(fn, "%s%02d", params.outputCFADFn, 11); // TN 041296
	fp = fopen(fn, "w");
	if (!fp)
	  {
	    fprintf(stderr, "Error opening CFAD ASCII output file.\n");
...
/* TN 041296
	sprintf(fn, "%s/%s.%s.%s.cfad%02d",
		...
		12);
*/
	sprintf(fn, "%s%02d", params.outputCFADFn, 12); // TN 041296
	fp = fopen(fn, "w");
	if (!fp)
	  {
	    fprintf(stderr, "Error opening CFAD ASCII output file.\n");
  ...
  if (params.ASCIIformat == 0)
      ...
      if (!fp)
	{
	  ...
	  TKreportWarning(W_2A55_OPEN_CFADRANGE_OFILE_FAILED);
	  exit(FAILED_CODE2);
	}
...
int Volume::readASCII(void)
{
  ...
  // generated from the reflectivity product 1C-51. 
  {
    ...
    if (params.ASCIIformat == 1)
      {
	sprintf(fn, "%s.%02d%02d%02d.%02d%02d.csmap", 
		params.siteName,
		params.year,
		params.month,
		params.day,
...
*/
	...
	if (!fp)
	  {
	    fprintf(stderr, "Error opening CFAD ASCII output file.\n");
		TKreportWarning(W_2A55_OPEN_CFAD_ASCII_IFILE_FAILED);
	    exit(FAILED_CODE1);
	  }
	for (l = 0; l <= cfad.max_Zindex; ++l)
	  {
