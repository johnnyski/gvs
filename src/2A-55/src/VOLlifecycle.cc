...
//                         * time      : time in HHMMSS Z format (000000 - 235959)
  ...
  if ((params.fpout1 = fopen(params.outputGeneralFn, "w")) == NULL)
    {
	  ...
	  TKreportWarning(W_2A55_OPEN_GEN_OFILE_FAILED);
      exit(FAILED_CODE3);
    }
...
Volume::Volume(int argc, char **argv)
{
  ...
  if (argc != 20)
    {
      fprintf(stderr,
	      ...
	      argv[0]);
	  TKreportWarning(W_2A55_BAD_USAGE2);
      exit(FAILED_CODE3);
    }
  ...
  sscanf(argv[15], "%2d%2d%2d", &params.hour, &params.minute, &params.second);
  ...
  sprintf(params.timeString, "%02d%02d%02d", params.hour, params.minute, params.second);
